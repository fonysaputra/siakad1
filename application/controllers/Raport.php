...
class Raport extends CI_Controller{
    ...
    function index(){
        ...
        function listnilaisiswa(){
            ...
            $data['rombel'] =   $this->db->query($rombel)->row_array();
            $data['siswa']  =   $this->db->query($siswa);
            $this->template->load('template','raport/list_nilaisiswa',$data);
        }
   ...
   function nilai_semester(){
       ...
       $siswa = $this->db->query($sqlSiswa)->row_array();
        ...
        $pdf->Cell(190,7,'SMK N 1 CANDIPURO',1,1,'C');
        $pdf->SetFont('Arial','',8);
        $pdf->Cell(190,5,'Kecamatan Kec. Candipuro, Kabupaten Kab. Lampung Selatan, Provinsi Prov. Lampung',1,1,'C');
        ...
        $pdf->Cell(40,5,'Rata Kelas',1,0,'L');
        $pdf->Cell(0,5,'',0,1);
        $pdf->SetFont('Arial','',9);
        ...
        foreach ($mapel as $m){
            ...
            $pdf->Cell(50,5,  Terbilang($nilai),1,0,'L');
            $pdf->Cell(23,5,  $this->ketercapaian_kopetensi($nilai),1,0,'L');
            $pdf->Cell(40,5,  ceil($this->rata_rata_nilai($m->id_jadwal)),1,0,'L');
            $pdf->Cell(0,5,'',0,1);
            $no++;
        }
        ...
        $pdf->Cell(190,5,'',0,1);
        $pdf->Cell(8, 5, 'No', 1,0);
        $pdf->Cell(50, 5, 'Mata Pelajaran', 1,0);
        $pdf->Cell(10, 5, 'Kkm', 1,0);
        $pdf->Cell(85, 5, 'Nilai', 1,0);
        $pdf->Cell(40, 5, 'Rata-Rata Kelas', 1,0);
        $pdf->Cell(0, 5, '', 0,1);
     ...
     function nilaisiswa(){
       ...
       $nim = $this->session->userdata('id_siswa');
        ...
        $data['nim']  =  $nim;
        $this->template->load('template','raport/nilai_siswa',$data);
